...
const onSpeechDetected = async(session, event) => {
  ...
  if (speech?.is_final) {
    ...
    try {
      ...
      if (responseText) {
        // Use say command instead of TTS tokens to avoid flush issues
        session
          .say({text: responseText})
          .send();
        
        logger.info(`session ${session.call_sid} sent response via say command`);
      ...
      session
        .say({text: "I'm sorry, I encountered an error processing your request. Please try again."})
        .send();
    }
